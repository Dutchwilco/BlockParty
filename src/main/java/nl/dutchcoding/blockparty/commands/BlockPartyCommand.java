...
public class BlockPartyCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleArenaCommand(CommandSender sender, String[] args) {
        ...
        switch (args[1].toLowerCase()) {
            ...
            case "info":
                if (args.length < 3) {
                    player.sendMessage("&cUsage: /bp arena info <arena>");
                    return;
                }
                handleArenaInfo(player, args[2]);
                break;
            default:
                player.sendMessage(plugin.getMessageManager().getMessage("arena-usage"));
                break;
    ...
    private void handleArenaInfo(Player player, String arenaName) {
        Arena arena = plugin.getArenaManager().getArena(arenaName);
        if (arena == null) {
            player.sendMessage(plugin.getMessageManager().getMessage("arena-not-found")
                .replace("{arena}", arenaName));
        ...
        player.sendMessage("&6=== Arena Info: &e" + arenaName + " &6===");
        player.sendMessage("&eStatus: &6" + arena.getSetupStatus());
        player.sendMessage("&eMin Players: &6" + arena.getMinPlayers());
        player.sendMessage("&eMax Players: &6" + arena.getMaxPlayers());

        if (arena.getPos1() != null) {
            player.sendMessage("&ePos1: &6" + formatLocation(arena.getPos1()));
        }
        if (arena.getPos2() != null) {
            player.sendMessage("&ePos2: &6" + formatLocation(arena.getPos2()));
        }
        if (arena.getSpawnLocation() != null) {
            player.sendMessage("&eSpawn: &6" + formatLocation(arena.getSpawnLocation()));
        }
        if (arena.getWaitLobby() != null) {
            player.sendMessage("&eLobby: &6" + formatLocation(arena.getWaitLobby()));
        }
    }
    ...
    private void sendHelpMessage(CommandSender sender) {
        ...
        if (sender.hasPermission("blockparty.admin")) {
            ...
            sender.sendMessage("§e/bp arena info <arena> §7- Show arena configuration status");
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args.length == 1) {
            ...
            completions.addAll(Arrays.asList("create", "pos1", "pos2", "setspawn", "waitlobby", "info"));
